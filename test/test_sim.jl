...
fit.loss[3] = 0.9
b = CINulator.multiplicativefitness(x[5][1], fit, log(2))
@test isapprox(b, (log(2) * (1 + 0.5)^2 * (1 -  0.9)))
...
fit.loss[3] = 0.9
b = CINulator.multiplicativefitness(x[5][1], fit, log(2))
@test isapprox(b, (log(2) * (1 + 0.5)^1 * (1 -  0.9) ^2))
...
b = log(2)
d = 0.0
μ = CINulator.Chrmutrate(Nchr, mutratesgain = [0.2, 0.2, 0.2, 0.0],
